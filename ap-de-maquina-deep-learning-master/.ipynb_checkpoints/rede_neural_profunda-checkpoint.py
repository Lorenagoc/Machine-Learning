...
relu = FuncaoAtivacao(lambda z: (z>0)*z,
                      lambda a,z,y,arr_dz_w_prox: (z>=0)*arr_dz_w_prox,
                      )



leaky_relu = FuncaoAtivacao(lambda z: (z>0)*z+(z<=0)*0.01*z,
                      lambda a,z,y,arr_dz_w_prox: (z>=0)*arr_dz_w_prox+(z<0)*0.01*arr_dz_w_prox,
                      )
...
class RedeNeural():
    ...
    def forward_propagation(self):
        ...
        for camada in range(num_camadas):
            if camada==0:
                self.arr_camadas[camada].forward_propagation(self.mat_x)
            else:
                self.arr_camadas[camada].forward_propagation(self.arr_camadas[camada -1].mat_a)
    ...
    def backward_propagation(self):
        ...
        """
        num_camadas = len(self.arr_camadas)
        camada = num_camadas - 1
        while camada > -1:
            self.arr_camadas[camada].backward_propagation(self.arr_y)
            camada -= 1 
